...
object TrieSpec extends Properties("Trie"):
  property("Empty trie has no strings") = forAll: (a: String) =>
    !Trie().contains(a)

  property(
    "Trie with inserted string starts with first character of said string"
  ) = forAll: (a: String) =>
    (a.length() > 0) ==> Trie().insert(a).startsWith(a.head.toString)

  property(
    "Trie with inserted string does not start with a different character"
